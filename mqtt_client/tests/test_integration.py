...
class IntegrationTest(unittest.TestCase):
    def test_round_trip_and_ha_recovery(self):
        ...
        try:
            ...
            self.assertIn((f"{prefix}/test/switch/state", "off"), messages)
            self.assertIn((f"{prefix}/test/switch/attribute/friendly_name", "Test Switch"), messages)
            probe.publish(f"{prefix}/test/switch/set", "ON", qos=1, retain=False)
            wait(lambda: (f"{prefix}/test/switch/state", "on") in messages)
